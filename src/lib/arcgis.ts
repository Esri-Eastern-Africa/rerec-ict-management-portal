import type { ArcGISQueryResponse, ArcGISEditResponse, ArcGISUser } from '@/types';
...
export async function generateToken(
  ...
  if (data.error) throw new Error(data.error.message || 'Failed to generate token');
  if (!data.token) throw new Error('Invalid credentials');
  return data.token as string;
}

export async function getPortalSelf(portalUrl: string, token: string): Promise<ArcGISUser> {
...
export async function getFeatureCount(layerUrl: string, token: string): Promise<number> {
  const params = new URLSearchParams({
    where: '1=1',
